...
use network::channel::{overflow_aware_compare, SequenceNr};
use playercmd::{ControllableComponent, PlayerCommand, run_command};
use cgmath::ApproxEq;
...
impl Prediction {
        ...
        self.predicted = Some(match self.predicted.take() {
            Some(mut entities) => {
                ...
                for &(_, cmd) in self.history.iter() {
                    run_command(cmd, &mut self.controllable, &mut entities);
                }
    ...
    pub fn predict(&mut self, cmd: PlayerCommand, sequence: SequenceNr) {
        ...
        self.predicted.as_mut().map(|ents| {
            run_command(cmd, &mut controllable, ents)
        });
